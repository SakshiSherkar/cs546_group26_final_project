...
const currentDate = new Date();
property.comments[reqCommentIndex].postTime = currentDate.toLocaleString('en-US', {month: '2-digit', day: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true});
await propertyCollection.updateOne({"comments.commentId": new ObjectId(commentId)}, {$set: {comments:property.comments}});
const updatedProperty = await productCollection.findOne({ "comments.commentId": new ObjectId(commentId)});
